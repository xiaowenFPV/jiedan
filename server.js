...
function seedData() {
    ...
    if (!fs.existsSync(path.join(DATA_DIR, 'tokens.json'))) {
        writeData('tokens', []);
    }
}
seedData();
...
app.delete('/api/services/:id', authMiddleware, (req, res) => {
    if (req.user.role !== 'dashi') return res.status(403).json({ error: '仅打手可下架服务' });
    
    const services = readData('services');
    const sid = parseInt(req.params.id);
    const s = services.find(s => s.id === sid);
    if (!s) return res.status(404).json({ error: '服务不存在' });
    if (s.dashiId !== req.user.dashiId) return res.status(403).json({ error: '只能下架自己的服务' });
